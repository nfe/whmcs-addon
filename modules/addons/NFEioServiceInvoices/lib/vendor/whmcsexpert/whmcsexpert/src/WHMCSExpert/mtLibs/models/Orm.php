...
class Orm extends Base
{
    ...
    static function getProperyColumn($property)
    {
        ...
        if (!isset($tableStructure['columns'][$property]['name'])) {
            throw new main\mtLibs\exceptions\System("Property $property not exists");
        }
    ...
    protected function getRawData($id, $haveToExits = true)
    {
        ...
        if (empty($data) && $haveToExits) {
            throw new main\mtLibs\exceptions\System('Unable to find ' . get_class($this) . ' with ID:' . $id);
        }
    ...
    {
        return main\mtLibs\process\MainInstance::I()->configuration()->tablePrefix . $table;
    }
    ...
    function fillProperties($data)
    {
        ...
        foreach ($structure['columns'] as $property => $configs) {
            if (!isset($used[$property]) && !isset($configs['notRequired'])) {
                throw new main\mtLibs\exceptions\System('Missing object property: ' . $property, main\mtLibs\exceptions\Codes::MISING_OBJECT_PROPERTY);
            }
        }
    }
